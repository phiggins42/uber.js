...
define(["has/dom", "./aop", "./dom"], function(has, aop, dom){
    ...
    function on(type, listener){
        ...
        if(this.attachEvent && createHandler){
            ...
            if(typeof node.uniqueNumber == "number" ||
                 ...
                 (node == win && (node == global || node.frameElement)) ||
                 // or if |this| is document
                 (global == node && node.nodeType == 9 && (node == document || node.frameElement))){
                // IE 8 and earlier
                var nodeId = dom.getNodeId(node),
                    nodeData = __cache__[nodeId],
                    existing = node[type],
    ...
    if(has.isHostType(document, "attachEvent") && has("dom-uniquenumber")){
        ...
        createHandler = function(id, event){
            return function(){
                if(id in __cache__ && event in __cache__[id]){
                    __cache__[id][event].call(this, dom.getWindow(dom.getDocument(this)).event);
                }
            };
        };
    }
