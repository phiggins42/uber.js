(function(global, define){
	define(["has", "uber/dom", "uber/deferred"], function(has, ubr){
		if(!has("dom")){ return ubr; }
		...
		ubr._listen = _listen;
		ubr._stopListening = _stopListening;

		ubr.listen = listen;
		ubr.stopListening = stopListening;
		...
		ubr.normalizeEventName = normalizeEventName;
		...
		(function(){
			...
			ubr.destroyElement = destroyElement;
			ubr.destroyDescendants = destroyDescendants;
		})();

		ubr.domReady = (function(){
		...
		ubr.preventDefault = preventDefault;
		ubr.stopPropagation = stopPropagation;
		ubr.stopEvent = stopEvent;

		ubr.getEventTarget = getEventTarget;
		ubr.getRelatedTarget = getRelatedTarget;

		ubr.normalizeEventName = normalizeEventName;
		ubr.fireEvent = fireEvent;
